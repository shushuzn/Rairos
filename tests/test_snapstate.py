...
class TestPaperSnapshot:
    ...
    def test_to_dict(self):
        ...
        assert set(d.keys()) == {
            "arxiv_id", "title", "abstract", "url",
            "extracted_text", "summary", "gaps_found", "notes",
        }
...
class TestGapSnapshot:
    ...
    def test_from_dict_round_trip(self):
        original = _make_gap(
            matched_papers=["p1"], archetype_match=0.5, accepted=True
        )
...
class TestSnapstateNewSession:
    ...
    @patch("llm.insight_evolution.get_evolution_tracker")
    def test_new_session_falls_back_to_tracker_archetype(self, mock_get_tracker, tmp_path):
        mock_get_tracker.return_value = self._mock_tracker()
        ss = Snapstate(base_dir=tmp_path)
