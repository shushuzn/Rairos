...
class TestAgentMessage:
    def test_fields(self):
        msg = AgentMessage(
            id="m1", topic="test", sender="alice",
            payload={"key": "value"}, timestamp=None, reply_to=None,
        )
        assert msg.id == "m1"
...
class TestBenchmarkRunner:
    def test_benchmark_result_init(self):
        r = BenchmarkResult(arxiv_id="test", test_dir="tests/", passed=10, failed=0, skipped=0, duration_seconds=1.5, passed_tests=[], failed_tests=[], error_message=None, gene_pool_entry=None, ruff_diagnostics=[], numerical_claims_total=0, numerical_claims_covered=0, coverage_ratio=0.8, covered_claims=[], uncovered_claims=[])
        assert r.passed == 10
        assert r.coverage_ratio == 0.8

    def test_benchmark_config_init(self):
        c = BenchmarkConfig(arxiv_id="test", paper_topic="ML", algorithm_description=" algo", test_dir="tests/", code_path="src/", code_quality=0.8, min_pass_rate=0.8, algorithm_fingerprint="abc", generated_code="def test(): pass", paper_section_refs={}, min_coverage_ratio=0.5, numerical_claims_total=5)
