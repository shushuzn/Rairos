...
class TestRenderText:
    ...
    def test_header_present(self):
        ...
        result = ComparisonResult(
            columns=[col], aspect_rows=[{"aspect": "Methods", "p1": "Transformer"}]
        )
    ...
    def test_long_value_truncated(self):
        ...
        result = ComparisonResult(
            columns=[col], aspect_rows=[{"aspect": "Methods", "p1": "X" * 50}]
        )
        output = self._render_text(result)
...
class TestRenderMarkdown:
    ...
    def _render_markdown(self, result) -> str:
        ...
        header = ["| Aspect |"] + [f"| {col.title[:40]} |" for col in result.columns]
    ...
    def _build_aspect_row(self, aspect, columns) -> dict:
        ...
        for col in columns:
            if aspect == "methods":
                ...
                row[col.paper_id] = ", ".join(f"{k}={v}" for k, v in col.metrics.items()) or "-"
            elif aspect == "authors":
                row[col.paper_id] = (
                    ", ".join(col.authors[:2]) + ("+" if len(col.authors) > 2 else "") or "-"
                )
            elif aspect == "year":
                row[col.paper_id] = str(col.year) if col.year else "-"
            elif aspect == "abstract":
                row[col.paper_id] = (
                    col.abstract[:100] + "..." if len(col.abstract) > 100 else col.abstract or "-"
                )
        return row
