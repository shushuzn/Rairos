...
from research_loop.claim_graph import (
    ClaimType, ComparisonOp, ClaimNode, ClaimEdge,
    Contradiction, BidirectionalContradiction, ClaimGraph,
)
...
class TestClaimNode:
    def test_fields(self):
        node = ClaimNode(
            claim_id="c1", paper_id="p1", claim_type=ClaimType.ACCURACY,
            value=95.0, comparison_op=ComparisonOp.GTE, source_text="accuracy is 95%",
        )
...
class TestClaimEdge:
    def test_fields(self):
        edge = ClaimEdge(
            from_paper="p1", to_paper="p2",
            claim_type=ClaimType.SPEEDUP, improvement_ratio=2.0,
...
class TestContradiction:
    def test_fields(self):
        c = Contradiction(
            ...
            metric="accuracy", description="Disagree on baseline",
            severity="high",
        )
...
class TestBidirectionalContradiction:
    def test_fields(self):
        ...
        bc = BidirectionalContradiction(
            ...
            edge_ab=edge1, edge_ba=edge2,
            severity="medium", description="Contradictory claims",
        )
...
class TestClaimGraph:
    ...
    def test_add_claim(self):
        g = ClaimGraph()
        g.add_claim(paper_id="p1", claim_type=ClaimType.ACCURACY, value=95.0, comparison_op=ComparisonOp.GTE, source_text="source")
        assert len(g.get_paper_claims("p1")) == 1

    def test_add_edge(self):
        g = ClaimGraph()
        g.add_edge(from_paper="p1", to_paper="p2", claim_type=ClaimType.SPEEDUP, improvement_ratio=2.0, source_text="faster")
