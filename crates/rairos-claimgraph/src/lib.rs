#![allow(deprecated)]
//! Cross-paper numerical claim tracking and contradiction detection
#![allow(dead_code)]
...
fn json_to_pyvalue(py: Python<'_>, v: &serde_json::Value) -> PyResult<Py<PyAny>> {
    match v {
        ...
        serde_json::Value::Array(arr) => {
            let list: Vec<Py<PyAny>> = arr
                .iter()
                .map(|item| json_to_pyvalue(py, item))
        ...
        serde_json::Value::Object(obj) => {
            ...
            for (k, val) in obj {
                dict.set_item(k, json_to_pyvalue(py, val)?)?;
            }
            Ok(dict.into())
        }
    }
...
#[pyfunction]
#[pyo3(signature = (action="status", paper_id=None, claim_type=None, value=None, source_text=None, from_paper=None, to_paper=None, improvement_ratio=None))]
pub fn claim_graph_action_py(
    action: &str,
    paper_id: Option<&str>,
    claim_type: Option<&str>,
    value: Option<f64>,
    ...
    Python::with_gil(|py| json_to_pyvalue(py, &result))
}

#[pyfunction]
pub fn render_claim_graph_html_py() -> PyResult<String> {
    Ok(render_claim_graph_html(None))
}

#[pyclass(name = "ClaimGraph")]
struct PyClaimGraph(ClaimGraph);
...
impl PyClaimGraph {
    ...
    fn load(_cls: &Bound<'_, PyType>) -> PyResult<Self> {
        Ok(Self(ClaimGraph::load(None)))
    }

    #[getter]
    fn edges<'a>(&self, py: Python<'a>) -> PyResult<Vec<Py<PyAny>>> {
        Ok(self.0.edges.iter().map(|e| {
            let dict = pyo3::types::PyDict::new(py);
            dict.set_item("from_paper", &e.from_paper).ok();
            dict.set_item("to_paper", &e.to_paper).ok();
            dict.set_item("claim_type", e.claim_type.as_str()).ok();
            dict.set_item("improvement_ratio", e.improvement_ratio).ok();
            dict.set_item("source_text", &e.source_text).ok();
            dict.into()
        }).collect())
    }

    fn get_inbound_improvement_claims(&self, paper_id: &str) -> PyResult<Vec<Py<PyAny>>> {
        let claims = self.0.get_inbound_improvement_claims(paper_id);
        Python::with_gil(|py| {
            Ok(claims.iter().map(|c| {
                let dict = pyo3::types::PyDict::new(py);
                dict.set_item("claim_id", &c.claim_id).ok();
                dict.set_item("paper_id", &c.paper_id).ok();
                dict.set_item("value", c.value).ok();
                dict.set_item("source_text", &c.source_text).ok();
                dict.into()
            }).collect())
        })
    }

    fn find_contradictions(&self) -> PyResult<Vec<Py<PyAny>>> {
        let contradictions = self.0.find_contradictions();
        Python::with_gil(|py| {
            Ok(contradictions.iter().map(|c| {
                let dict = pyo3::types::PyDict::new(py);
                dict.set_item("paper_a", &c.claim_a.paper_id).ok();
                dict.set_item("paper_b", &c.claim_b.paper_id).ok();
                dict.set_item("metric", &c.metric).ok();
                dict.set_item("description", &c.description).ok();
                dict.set_item("severity", &c.severity).ok();
                dict.into()
            }).collect())
        })
    }
...
#[pymodule]
pub fn rairos_claimgraph_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(claim_graph_action_py, m)?)?;
    m.add_function(wrap_pyfunction!(render_claim_graph_html_py, m)?)?;
    m.add_class::<PyClaimGraph>()?;
    Ok(())
}
