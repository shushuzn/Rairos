...
impl ToolHandler for PaperSlidesHandler {
    ...
    async fn call(&self, params: Value) -> Result<Value, String> {
        ...
        // Background/context (2-3 slides for longer talks)
        ...
        // References (optional for longer talks)
        if talk_type == "seminar" || talk_type == "defense" {
            slides.push(Slide {
                number: slide_num,
...
// ─── Paper Grant Proposal Generator ──────────────────────────────────────────
...
fn build_proposal_markdown(
    ...
    let mut md = String::new();
    let today = chrono::Local::now().format("%Y-%m-%d").to_string();
    let (__, _, agency_name) = agency_config(agency);
...
fn get_test_recommendations(data_type: &str, groups: &str, hypothesis: &str) -> Vec<TestRecommendation> {
    ...
    match (data_type, groups, hypothesis) {
        // Continuous + 2 groups + difference
    ...
    // Add common tests for multiple groups with repeated measures
