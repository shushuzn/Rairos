...
impl LitReviewGenerator {
    ...
    fn build_papers_text(&self, papers: &[Paper]) -> String {
        ...
        for (i, p) in papers.iter().enumerate().take(30) {
            let authors = if !p.authors.is_empty() {
                p.authors.iter().take(3).cloned().collect::<Vec<_>>().join(", ")
            } else {
                "Unknown".to_string()
            };
    ...
    fn generate_template_review(&self, topic: &str, papers: &[Paper]) -> LitReview {
        ...
        let min_year = years.iter().min().cloned().unwrap_or_else(|| "N/A".to_string());
        let max_year = years.iter().max().cloned().unwrap_or_else(|| "N/A".to_string());

        let mut scored_papers = papers.to_vec();
        scored_papers.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
    ...
    fn render_markdown(&self, review: &LitReview) -> String {
        ...
        lines.push(format!("_Generated by Rairos LitReviewGenerator on {}_", now));

        lines.join("\n")
    }

    fn save_review(&self, review: &LitReview, markdown: &str, output_dir: &Path) -> Option<PathBuf> {
        ...
        let filename = format!("litreview_{}_{}.md", slug, &uuid::Uuid::new_v4().to_string()[..6]);
...
mod tests {
    ...
    fn test_build_papers_text() {
        let generator = LitReviewGenerator::new();
        let papers = vec![
            make_paper("2301.00001", "Test Paper", "2023-06-15", 0.9),
        ];
