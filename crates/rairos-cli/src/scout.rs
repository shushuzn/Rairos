...
pub fn scout(
    ...
    if sources == "news" || sources == "all" {
        for (feed_name, feed_url) in NEWS_FEEDS {
            ...
            let articles = rt.block_on(fetch_rss_feed(feed_url, feed_name, 5));
            for p in articles {
...
pub fn get_topics_from_capsules(capsules: &[CapsuleStub]) -> Vec<String> {
    ...
    for pair in top_kws.chunks(2) {
        if pair.len() == 2 {
            topics_set.insert(format!("{} {}", pair[0], pair[1]));
        }
    }

    let mut topics_vec: Vec<String> = topics_set.into_iter().collect();
