...
pub fn handle_codegraph_files() -> Result<()> {
    tokio::runtime::Handle::current().block_on(async {
        let files = with_codegraph(|g| {
            Ok(tokio::runtime::Handle::current()
                .block_on(async { g.files().await })?)
        })
        .await?;
...
pub fn handle_codegraph_search(query: &str) -> Result<()> {
    tokio::runtime::Handle::current().block_on(async {
        let results = with_codegraph(|g| {
            Ok(tokio::runtime::Handle::current()
                .block_on(async { g.search(query, 20).await })?)
        })
        .await?;
...
pub fn handle_codegraph_node(node_id: i64) -> Result<()> {
    tokio::runtime::Handle::current().block_on(async {
        let node = with_codegraph(|g| {
            Ok(tokio::runtime::Handle::current()
                .block_on(async { g.get_node(node_id).await })?)
        })
        .await?;
...
pub fn handle_codegraph_callers(node_id: i64, depth: usize) -> Result<()> {
    tokio::runtime::Handle::current().block_on(async {
        let callers = with_codegraph(|g| {
            Ok(tokio::runtime::Handle::current()
                .block_on(async { g.get_callers(node_id, depth).await })?)
        })
        .await?;
...
pub fn handle_codegraph_callees(node_id: i64, depth: usize) -> Result<()> {
    tokio::runtime::Handle::current().block_on(async {
        let callees = with_codegraph(|g| {
            Ok(tokio::runtime::Handle::current()
                .block_on(async { g.get_callees(node_id, depth).await })?)
        })
        .await?;
