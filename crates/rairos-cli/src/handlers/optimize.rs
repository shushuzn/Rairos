...
pub fn handle_code_gene_workflow(
    ...
    for gene in &genes {
        ...
        // Search existing files for relevant code
        ...
        // Build PR description with plan
...
"#,
        ...
        // Step 3: Implement (if approved and requested)
        if auto_implement && result.status == "approved" {
            ...
            std::process::Command::new("git")
                .args(&["commit", "-m", &format!("code-gene: {} - {}", gene.gap_type, short_id)])
                .output()
                .ok();
            ...
            // Create PR with plan in description
            let pr_title = format!("[Gene] {} - {}", gene.gap_type, gene.target_crate);

            let pr_output = std::process::Command::new("gh")
                .args(&["pr", "create", "--repo", repo, "--title", &pr_title, "--body", &pr_description])
                .output()
                .ok();

            let pr_url = pr_output.and_then(|o| {
                let stdout = String::from_utf8_lossy(&o.stdout);
                stdout.lines().find(|l| l.starts_with("https://")).map(|s| s.to_string())
            });

            result.pr_url = pr_url;
            result.impl_branch = Some(branch_name.clone());
            ...
            std::process::Command::new("git")
                ...
                .output()
                .ok();
        } else if result.status == "needs_revision" || result.status == "approved" {
            // Switch back to main if we didn't implement
            std::process::Command::new("git")
                .args(&["checkout", "main"])
                .output()
                .ok();
