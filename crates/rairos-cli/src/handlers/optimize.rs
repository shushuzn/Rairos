...
/// Run complete workflow: plan → review → approve → implement
pub fn handle_code_gene_workflow(
    ids: &str,
    repo: &str,
    skip_review: bool,
    auto_approve: bool,
    ...
    let steps = if auto_implement { 4 } else { 3 };
    ...
    struct WorkflowResult {
        ...
        impl_branch: Option<String>,
        error: Option<String>,
    }

    let pb = ProgressBar::new(genes.len() as u64 * steps as u64);
...
_Generated by code-gene workflow_"#,
        ...
        // Step 2: LLM Review (if not skipped)
        pb.set_message(format!("review:{}", short_id));
        pb.inc(1);

        let review_verdict = if !skip_review {
...
"#,
                ...
                if let Ok(rt) = rt {
                    let review = rt.block_on(async {
                        ...
                        let messages = vec![
                            std::collections::HashMap::from([
                                ("role".to_string(), "user".to_string()),
                                ("content".to_string(), review_prompt),
                            ]),
                        ];
                        client.chat_completions(messages, None, None, false).await
                    });
                    ...
                    if let Ok(review) = review {
                        if review.contains("VERDICT: APPROVE") {
                            "approve"
                        } else {
                            "reject"
                        }
        ...
        // Step 3: Auto-approve (if requested and approved by LLM)
        pb.set_message(format!("approve:{}", short_id));
        pb.inc(1);

        if auto_approve && (review_verdict == "approve" || skip_review) {
            updated_gene.status = "approved".to_string();
            updated_gene.status_history.push(StatusChange {
        ...
        } else if review_verdict == "reject" {
            updated_gene.status = "needs_revision".to_string();
            updated_gene.status_history.push(StatusChange {
                from_status: "planned".to_string(),
                to_status: "needs_revision".to_string(),
                reason: "LLM review rejected".to_string(),
                timestamp: chrono_now(),
            });
            save_code_capsule(&updated_gene).ok();
            result.status = "needs_revision".to_string();
        }

        // Step 4: Auto-implement (if approved and requested)
        if auto_implement && result.status == "approved" {
            pb.set_message(format!("impl:{}", short_id));
            pb.inc(1);

            let impl_branch_name = format!("impl/code-gene/{}", short_id);

            std::process::Command::new("git")
                .args(&["checkout", "-b", &impl_branch_name])
                .output()
                .ok();

            // Write implementation
            let target_path = format!("crates/{}/src/lib.rs", gene.target_crate);
            if let Ok(existing) = std::fs::read_to_string(&target_path) {
                ...
                std::fs::write(&target_path, new_code).ok();
            }
            ...
            std::process::Command::new("git")
                .args(&["commit", "-m", &format!("impl: {} - {}", gene.gap_type, short_id)])
                .output()
                .ok();

            std::process::Command::new("git")
                .args(&["push", "-u", "origin", &impl_branch_name, "--force"])
                .output()
                .ok();

            // Create impl PR
            let impl_pr_title = format!("[Impl] {} - {}", gene.gap_type, gene.target_crate);
            let impl_pr_body = format!(
                r#"## Code Gene Implementation PR
...
_Generated by code-gene workflow_"#,
            ...
            let impl_pr_output = std::process::Command::new("gh")
                .args(&["pr", "create", "--repo", repo, "--title", &impl_pr_title, "--body", &impl_pr_body])
                .output()
                .ok();

            let impl_pr_url = impl_pr_output.and_then(|o| {
                let stdout = String::from_utf8_lossy(&o.stdout);
                stdout.lines().find(|l| l.starts_with("https://")).map(|s| s.to_string())
            });

            result.pr_url = impl_pr_url.or(result.pr_url);

            updated_gene.status = "implemented".to_string();
            updated_gene.status_history.push(StatusChange {
                from_status: "approved".to_string(),
                to_status: "implemented".to_string(),
                reason: "Auto-implemented via workflow".to_string(),
                timestamp: chrono_now(),
            });
            save_code_capsule(&updated_gene).ok();
            result.status = "implemented".to_string();
            result.impl_branch = Some(impl_branch_name);
        } else {
            pb.inc(1);
        }
