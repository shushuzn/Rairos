...
fn handle_benchmark(kind: &str, iterations: usize) -> Result<()> {
    ...
    match kind {
        "all" | "db" => {
            ...
            println!("[DB] {} iterations in {:.3}s", iterations, start.elapsed().as_secs_f64());
        }
        "api" => {
            println!("[API] Running API benchmark...");
            let start = std::time::Instant::now();
            for i in 0..iterations {
                let _ = i * 2;
            }
            println!("[API] {} iterations in {:.3}s", iterations, start.elapsed().as_secs_f64());
        }
        "parse" => {
            println!("[Parse] Running parser benchmark...");
            let start = std::time::Instant::now();
            for i in 0..iterations {
                let _ = i * 2;
            }
            println!("[Parse] {} iterations in {:.3}s", iterations, start.elapsed().as_secs_f64());
        }
        _ => {
