...
rairos import 2604.28192

# 2. Search for related work
rairos research "<topic>" --limit 10 --no-ai

# 3. Detect research gaps
rairos gap --no-llm "<topic>"

# 4. Run deep research agent (if LLM available)
rairos agent deep-research "<question>" --iterations 3

# 5. Set up subscription for continuous monitoring
rairos subscribe add "<topic>"
...
**Rairos pipeline:**
1. `rairos research "LAPO PPO comparison latent reasoning" --limit 15 --no-ai`
2. Import key comparison papers: PPO-based VLA methods, latent reasoning papers
3. `rairos gap extract <paper_id>` on each to extract claimed convergence rates
...
**Rairos pipeline:**
1. `rairos gap "adaptive latent chain length reasoning robotics" --no-llm`
...
**Rairos pipeline:**
1. `rairos research "zero-shot generalization robot manipulation latent reasoning" --no-ai`
2. Import LaST-R1 + all papers studying VLA generalization
3. Extract gap: `rairos gap extract <id>` on each generalization study
...
**Rairos pipeline:**
1. `rairos gap "reasoning action interleaving multi-step robot" --no-llm`
2. Import hierarchical planning + VLA papers
3. Use `rairos insight` to tag papers by their interleaving strategy
4. `rairos subscribe add "hierarchical VLA reasoning"` for continuous monitoring
...
**Rairos pipeline:**
1. `rairos research "visual vs physical latent attention VLA" --no-ai`
2. Import LaST-R1 + interpretability papers
3. Extract gap: `rairos gap --no-llm "latent representation analysis VLA"`
4. Build contradiction map: `rairos gap contradictions`
...
**Rairos pipeline:**
1. `rairos research "warmup policy quality diversity VLA RL" --no-ai`
...
**Rairos pipeline:**
1. `rairos gap extract 2604.28192` — extract LaST-R1's own claims about LIBERO
2. `rairos research "LIBERO benchmark limitations robotics" --no-ai`
...
**Rairos pipeline:**
1. `rairos research "latent reasoning action conditioned policy robot" --no-ai`
2. Import non-VLA architectures: Diffusion Policy, ACT, π0
3. `rairos insight rate --card <id> --stars <N>` — tag by architecture type
4. `rairos gap --no-llm "LAPO transfer non-VLA"` to detect missing evidence
...
**Rairos pipeline:**
1. `rairos gap extract <id>` on LaST-R1 + interpretability papers
2. `rairos research "human intervention latent reasoning corrigibility" --no-ai`
3. Import papers on: mechanistic interpretability, human-in-the-loop robotics
4. `rairos gap contradictions` — find papers that argue for vs against human intervention
...
- [ ] Q6: Attention 分布差异 — pipeline: `rairos gap contradictions`
- [ ] Q7: 热启动策略 — pipeline: `rairos research "warmup VLA RL" --no-ai`
- [ ] Q8: LIBERO 覆盖分析 — pipeline: `rairos research "LIBERO limitations" --no-ai`
- [ ] Q9: 非 VLA 迁移 — pipeline: `rairos insight rate`
- [ ] Q10: 人类干预 — pipeline: `rairos gap contradictions`
