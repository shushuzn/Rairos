...
### Engineering Quality
- **Pre-commit hooks**: `.pre-commit-config.yaml` with ruff check + ruff format on every commit
- **Justfile overhaul**: add `fmt-check`, `typecheck`, `ci`, `ci-full`, `hooks` recipes; fix `--cov` target
- **Branch protection**: main requires `commitlint`, `lint-and-crossref`, `typecheck` status checks
- **CI green**: ruff format 23 files; all 498 files now pass format check
- **pyproject.toml**: bump v1.5.4→v1.7.0, collapse blank lines, add Python 3.13 classifier
- **Root cleanup**: remove 35 zero-byte type-name junk files; delete tracked `null` file
- **.gitignore**: deduplicate entries, add SITUATION_REPORT.md

### Gene Pool
- **Saturation fix**: relax duplicate check from exact match to action_gap_title + keyword overlap (<80%)
- **Cross-topic seeds**: inject high-quality capsules from other topics when propose() generates <2 candidates
- **Diversity expansion**: 49→112 capsules across 36 topics; avg score 0.666→0.710
- **gap_composite_score**: Gene Pool as primary sort dimension (40% gene_pool + 20% trend + 20% pref)
- **GenePool–DeepResearch integration**: PLANNER queries GenePool via `find_capsule()` before gap analysis
- **Evolve loop**: audit→propose→evaluate→apply verified end-to-end with real LLM calls

### paper2code Pipeline
- Fix 4 bugs in one session: hp regex newline matching, hp_val repr(), torch importorskip, CRLF corruption
- Secondary prose-stripping: MiniMax model plain-text preamble detection + stripping
- Closing fence regex fix: `\n\`\`\`\s*$`→`\n\s*\`\`\`\s*\n`
- Pipeline verification: 4/4 tests passed, end-to-end `PaperPipeline().run('2106.09685')` successful

### Bug Fixes
- **Deep Research Agent**: fix `p.arxiv_id`→`p.uid` (Paper), `add_papers`→`upsert_paper` (Database), snapshot uid fix
- **Event analysis**: multi-backend paper search (arXiv→CrossRef→Semantic Scholar) with 429 retry
- **MiniMax thinking tags**: strip `<think>..</think>` blocks + markdown JSON prefix from `client.generate()`
- **queue --list**: fix `parse_status` vs `job_queue` confusion; add `--pending` flag
- **CLI dual registration**: fix `_run_kg` re-export path after subdir reorganization
- **mypy**: resolve all type errors across `llm/` and `research_loop/` modules

### Features

- **Web UI Innovation Pages**: Add Gene Pool, Gap Detection, and InsightEvolution pages to web UI
  - 🧬 **Gene Pool page**: 48 capsules visualization with signal strength, quality distribution (GOOD×20 / FAIR×28), accept/reject recording, quality/theme filtering
  - 🎯 **Gap Detection page**: 6-dimension tuple sorting visualization, accept/reject feedback loop, gene_pool_score per gap
  - 🔄 **InsightEvolution page**: Full feedback-descent闭环 (Audit→Propose→Evaluate→Apply), ASCII architecture diagram, audit result display

## v1.5.5 (2026-05-01)

### Features

- **Deep Research Agent**: New research_loop/deep_research.py with iterative research loop, archetype-aware query reformulation, and gap-driven refinement
- **Snapstate Persistence**: research_loop/snapstate.py enables pause/resume for long-running research sessions via gene_pool.jsonl snapshots
- **Agent CLI**: New 'agent' subcommand with 6 subcommands: deep-research, list-sessions, resume, continue, inspect, review; enables interactive deep research workflows
- Archetype-aware query reformulation: DeepResearchAgent boosts method_focused archetype when GapAnalyzer detects theoretical gaps
- All CLI commands now registered in cli/_registry.py with dynamic loading
...
- Restore `core/rate_limiter.py` (297 lines, previously deleted by mistake)
- Clean README: remove stale badges, add star history SVG

### Features

- **Gene/Capsule Storage System**: Encode successful gap accept patterns as retrievable `CapsuleGene` objects persisted to `gene_pool.jsonl`
- **Gap Feedback Loop**: `record_gap_accept` / `record_gap_reject` now automatically encode/decode Capsule patterns; CLI `gap --feedback` interactive mode collects accept/reject signals
- Archetype radar: `get_archetype()` returns 6D research personality vector; `render_archetype_radar()` renders ASCII radar chart
