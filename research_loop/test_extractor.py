...
def _fw_import(framework: str, lib: str = "") -> str:
    """Get the standard import statement for a framework."""
    if lib == "torch" or framework == "pytorch":
        return "import torch" if framework == "pytorch" else f"import {lib}"
    return f"import {lib}"


def _fw_module(framework: str) -> str:
    """Get the module name for tensor operations."""
    return {"pytorch": "torch", "jax": "jnp", "numpy": "np"}.get(framework, "torch")


def _fw_randn(framework: str) -> str:
    """Get a random tensor creation expression for this framework."""
...
def _fw_fixtures(framework: str) -> str:
    """Render framework-appropriate conftest.py fixtures."""
    if framework == "pytorch":
        return """\
@pytest.fixture
def sample_input():
...
def sample_input():
    \"\"\"Create a sample input array for testing.\"\"\"
    return np.random.randn(2, 8, 512).astype(np.float32)
...
def extract_tests(content: Any, code: str, module_name: str = "model", framework: str = "pytorch") -> TestSuite:
    ...
    """
    base_imports = FRAMEWORK_IMPORTS.get(framework, FRAMEWORK_IMPORTS["pytorch"])
    suite = TestSuite(
        arxiv_id=content.arxiv_id,
        module_name=module_name,
        imports=["import pytest"] + base_imports + [f"from {module_name} import *"],
    )
...
def _render_conftest(suite: TestSuite, framework: str = "pytorch") -> str:
    """Render conftest.py with any shared fixtures."""
    fw_imports = "\n".join(FRAMEWORK_IMPORTS.get(framework, FRAMEWORK_IMPORTS["pytorch"]))
    fw_fixtures = _fw_fixtures(framework)
    additional = ""
...
{fw_fixtures}{additional}
"""


def _render_test_module(suite: TestSuite, framework: str = "pytorch") -> str:
    ...
    fw_imports = FRAMEWORK_IMPORTS.get(framework, FRAMEWORK_IMPORTS["pytorch"])
    stdlib_imports = ["import pytest"] + fw_imports
