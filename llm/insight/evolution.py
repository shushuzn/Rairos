...
{{"winner": "A" or "B", "reasoning": "why", "confidence": 0.0-1.0}}"""
    ...
    def _load_capsules(self) -> List[CapsuleGene]:
        """Load capsules via the tracker (delegates to CapsuleStorageMixin)."""
        return self.tracker._load_capsules()

    def _save_capsules(self, capsules: List[CapsuleGene]) -> None:
        """Save all capsules via the tracker (delegates to CapsuleStorageMixin)."""
        self.tracker._save_capsules(capsules)
    ...
    def _merge_capsules(self, capsules: List[CapsuleGene]) -> Tuple[int, List[CapsuleGene]]:
        ...
        for loser_id in to_archive:
            loser_capsule: CapsuleGene | None = next(
                (c for c in capsules if c.capsule_id == loser_id), None
            )
            if loser_capsule:
                winner_id = next(
                    (
