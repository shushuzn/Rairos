...
{{"winner": "A" or "B", "reasoning": "why", "confidence": 0.0-1.0}}"""
    ...
    def _load_capsules(self) -> List[CapsuleGene]:
        """Load all capsules from SQLite (auto-migrates from JSONL on first use).

        Uses a fresh SQLite connection each call for test isolation.
        """
        from llm.insight.gene import CapsuleGene
        import json as _json
        import sqlite3

        db_path = self.tracker.data_dir / "gene_pool.db"
        jsonl_path = self.tracker.data_dir / "gene_pool.jsonl"
        ...
        try:
            conn.executescript("""
                CREATE TABLE IF NOT EXISTS capsules (
                    capsule_id TEXT PRIMARY KEY, created_at TEXT NOT NULL DEFAULT '',
                    trigger_topic TEXT NOT NULL DEFAULT '', trigger_gap_type TEXT NOT NULL DEFAULT '',
                    trigger_keywords TEXT NOT NULL DEFAULT '[]', action_gap_type TEXT NOT NULL DEFAULT '',
                    action_gap_title TEXT NOT NULL DEFAULT '', outcome_success_score REAL NOT NULL DEFAULT 0.5,
                    feedback_count INTEGER NOT NULL DEFAULT 0, evolved_generation INTEGER NOT NULL DEFAULT 0,
                    archetype TEXT NOT NULL DEFAULT '{}', status TEXT NOT NULL DEFAULT 'active',
                    low_score_streak INTEGER NOT NULL DEFAULT 0, credibility_score REAL NOT NULL DEFAULT 0.5,
                    trendslop INTEGER NOT NULL DEFAULT 0, trendslop_reason TEXT NOT NULL DEFAULT '',
                    ...
                    with open(jsonl_path, encoding="utf-8") as f:
                        for line in f:
                            ...
                            try:
                                ...
                                cap = CapsuleGene.from_dict(data)
                                conn.execute(
                                    """INSERT OR REPLACE INTO capsules VALUES (
                                        ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?
                                    )""", (
                                        cap.capsule_id, cap.created_at, cap.trigger_topic,
                                        cap.trigger_gap_type, _json.dumps(cap.trigger_keywords),
                                        cap.action_gap_type, cap.action_gap_title,
                                        cap.outcome_success_score, cap.feedback_count,
                                        cap.evolved_generation, _json.dumps(cap.archetype),
                                        cap.status, cap.low_score_streak, cap.credibility_score,
                                        1 if cap.trendslop else 0, cap.trendslop_reason,
                                        cap.credibility_badge, cap.source_arxiv_category,
                                    )
                                )
                                migrated += 1
                            except Exception:
            ...
            result = []
            for r in rows:
                result.append(CapsuleGene(
                    capsule_id=r["capsule_id"], created_at=r["created_at"],
                    trigger_topic=r["trigger_topic"], trigger_gap_type=r["trigger_gap_type"],
                    trigger_keywords=_json.loads(r["trigger_keywords"]),
                    action_gap_type=r["action_gap_type"], action_gap_title=r["action_gap_title"],
                    outcome_success_score=r["outcome_success_score"],
                    feedback_count=r["feedback_count"], evolved_generation=r["evolved_generation"],
                    archetype=_json.loads(r["archetype"]), status=r["status"],
                    low_score_streak=r["low_score_streak"],
                    credibility_score=r["credibility_score"], trendslop=bool(r["trendslop"]),
                    trendslop_reason=r["trendslop_reason"], credibility_badge=r["credibility_badge"],
                    source_arxiv_category=r["source_arxiv_category"],
                ))
            return result
        finally:
            conn.close()
    ...
    def _save_capsules(self, capsules: List[CapsuleGene]) -> None:
        """Save all capsules to SQLite. Uses a fresh connection for test isolation."""
        import json as _json
        import sqlite3
            ...
            for c in capsules:
                conn.execute(
                    """INSERT OR REPLACE INTO capsules VALUES (
                        ...
                        c.capsule_id, c.created_at, c.trigger_topic,
                        c.trigger_gap_type, _json.dumps(c.trigger_keywords),
                        c.action_gap_type, c.action_gap_title,
                        c.outcome_success_score, c.feedback_count,
                        c.evolved_generation, _json.dumps(c.archetype),
                        c.status, c.low_score_streak, c.credibility_score,
                        1 if c.trendslop else 0, c.trendslop_reason,
                        c.credibility_badge, c.source_arxiv_category,
                    )
                )
            conn.commit()
        finally:
