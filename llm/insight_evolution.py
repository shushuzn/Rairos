...
class CapsuleGene:
    ...
    def trigger_match(self, topic: str, gap_type: str, keywords: List[str]) -> float:
        ...
        if keywords and self.trigger_keywords:
            ...
            overlap = set(k.lower() for k in keywords) & set(k.lower() for k in self.trigger_keywords)
...
class EvolutionTracker:
    ...
    def encode_capsule(
        ...
        Called when the user accepts a gap — that accept signals a successful
    ...
    def find_capsule(
        ...
        """Find capsules from the Gene Pool that match the given context.

        Used by GapAnalyzerV2 to inject successful past patterns into gap ranking.
        """
