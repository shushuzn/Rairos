...
DEPLOYMENT_KEYWORDS = [
    "deployment", "deployed", "in production", "in deployment",
    "real-world", "field trial", "pilot program", "operational",
]
...
def check_eval_gaps() -> Dict[str, Any]:
    ...
    for cat, cat_papers in by_category.items():
        ...
        for p in cat_papers:
            ...
            if yr:
                deploying.append({"title": p.get("title", ""), "year": yr, "paper_id": p.get("id", "")})
...
def render_eval_gap_html(data: Optional[Dict[str, Any]] = None) -> str:
    ...
    if not alerts:
        lines.append("<p>No evaluation gaps detected. Deployment timelines appear adequately covered by benchmark research.</p>")
    else:
        for alert in alerts:
            color = "#C4706A" if alert["severity"] == "high" else "#D4A055"
            lines.append(f"<div style='border-left: 4px solid {color}; padding: 10px 14px; margin-bottom: 14px; background: rgba(0,0,0,0.02);'>")
            lines.append(f"<div style='font-weight:700;font-size:14px;color:#2a2a2a'>{alert['category']}</div>")
            lines.append(f"<div style='font-size:12px;color:#7a7570;margin:4px 0'>"
            ...
            for dep in alert.get("deploying_papers", [])[:2]:
                lines.append(f"<div style='font-size:11px;color:#A89E8C;margin-left:8px'>• {dep['title'][:70]}</div>")
            lines.append("</div>")
