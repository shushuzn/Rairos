    def gap_composite_score(gap: ResearchGapV2) -> tuple:
            """Normalized composite score for gap ranking.

            Gene Pool signal is primary since it represents demonstrated success.
            If Gene Pool has a high-scoring capsule matching this gap, that
            outranks all theoretical/trend-based scoring.

            Returns (gene_pool_primary, composite_tiebreaker) so Gene Pool score
            dominates lexicographically while composite allows fine-grained ranking.
            """
            gap_type_str = gap.gap_type.value

            # Gene Pool: primary signal (demonstrated success evidence)
            gene_pool = gap.gene_pool_score  # 0.0-1.0

            # Preference: user historical taste
            ...
            # Trend: normalize 0-2 to 0-1
            trend_normalized = min(gap.novelty_score / 2.0, 1.0)

            # Severity: normalize 0-3 to 0-1
            severity_order = {GapSeverity.HIGH: 3, GapSeverity.MEDIUM: 2, GapSeverity.LOW: 1}
            severity_normalized = severity_order.get(gap.severity, 0) / 3.0

            # Keyword: normalize 0-3 to 0-1
            ...
            keyword_normalized = min(max(kw_raw / 3.0, 0.0), 1.0)

            # Composite: Gene Pool is primary signal (40%)
            composite = (
                0.40 * gene_pool +
        ...
        return gaps, has_preferences"""Enhanced Gap Analyzer: Multi-source gap detection with insights fusion."""
...
class GapAnalyzerV2(GapDetector):
    ...
    def _apply_preference_sorting(
        ...
        def gap_composite_score(gap: ResearchGapV2) -> tuple:
            """Composite sort key: Gene Pool primary, trend secondary, all normalized.

            Returns (gene_pool_score, trend_normalized, composite_score).
            - gene_pool_score (0.0-1.0): Gene Pool signal — dominant dimension.
            - trend_normalized (0.0-1.0): novelty_score/2, normalized to comparable scale.
            - composite_score: weighted sum; disliked/deprioritized gaps get -0.2 penalty.

            Gene Pool is the primary sort key because it encodes historically successful
            research patterns. This makes it the dominant signal in gap prioritization.
            """
            gap_type_str = gap.gap_type.value

            # Gene Pool signal: success pattern match (0.0-1.0) — PRIMARY
            gene_pool_score = gap.gene_pool_score

            # Trend score normalized to 0-1 scale (original is 0-2)
            trend_score = gap.novelty_score  # 0-2 scale
            trend_normalized = trend_score / 2.0

            # Preference score for display
            numeric_score = self.evolution_tracker.get_gap_type_score(gap_type_str)
            pref_score = 0.0
            if gap_type_str in preferred_types:
                pref_score = 0.2
            ...
            # Severity score (0-1)
            severity_order = {GapSeverity.HIGH: 1.0, GapSeverity.MEDIUM: 0.5, GapSeverity.LOW: 0.0}
            severity_score = severity_order.get(gap.severity, 0.0)

            # Keyword score (0-1)
            gap_kws = _extract_gap_keywords(gap.title)
            keyword_score = min(sum(
                self.evolution_tracker.get_keyword_score(kw)
                for kw in gap_kws if kw in top_keywords
            ) / 3.0, 1.0)  # normalize to 0-1, max +3 → cap at 1.0

            # Composite: Gene Pool dominant (40%), rest weighted
            composite = (
                0.40 * gene_pool_score +
                0.20 * trend_normalized +
                0.20 * pref_score +
                0.12 * severity_score +
                0.08 * keyword_score
            ...
            return (gene_pool_score, trend_normalized, composite)
