        ...
        for j in range(i + 1, len(types)):
            ...
            for ca in by_type[type_a][:5]:
                for cb in by_type[type_b][:5]:
                    shared = set(k.lower() for k in ca.trigger_keywords) & set(
                        k.lower() for k in cb.trigger_keywords
                    )
                    if len(shared) >= 2:
                        bridges.append(
                            {
                                ...
                                "strength": round(
                                    len(shared)
                                    / max(len(set(ca.trigger_keywords + cb.trigger_keywords)), 1),
                                    2,
                                ),
                            }
                        )
...
def render_html(bridges):
    ...
    for b in bridges[:20]:
        ...
        html.append(
            f'<div style="font-size:11px;color:#888;">{b["type_a"]} ↔ {b["type_b"]} (strength={b["strength"]})</div>'
        )
        ...
        html.append(
            f'<div style="font-size:11px;color:#888;">shared: {", ".join(b["shared_keywords"])}</div>'
        )
        html.append("</div>")
    html.append("</div>")
    return "\n".join(html)
