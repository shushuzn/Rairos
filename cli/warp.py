...
class _Colors:
    ...
    def __init__(self) -> None:
        ...
        self._aliases = {
            ...
            'code_bg': 'RESET_BG', 'link': 'LINK', 'header': 'HEADER',
            'list': 'LIST', 'quote': 'QUOTE', 'reset': 'RESET',
            'syn_keyword': 'SYN_KEYWORD', 'syn_string': 'SYN_STRING',
...
def _tokenize_generic(code: str) -> str:
    ...
    for line in lines:
        if ' #' in line or line.lstrip().startswith('#'):
            ...
            if idx >= 0:
                result_lines.append(
                    f"{line[:idx]}{_C.SYN_COMMENT}{line[idx:]}{_C.RESET}"
                )
                continue
        result_lines.append(line)
