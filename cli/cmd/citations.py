...
def _run_citations(args: argparse.Namespace) -> int:
    ...
    if args.citation_from and args.citation_to:
        ...
        if args.format == "csv":
            ...
            if via_papers:
                ...
                title_map = {
                    pid: (paper_map[pid].title or "") for pid in via_ids if pid in paper_map
                }
                for c in via_papers:
            ...
            if via_papers:
                ...
                for via in via_papers:
                    ...
                    body_lines.append(
                        f"  [#D0D1FE]{paper_from}[/] → [#FF8272]{via.target_id}[/] → [#FF8272]{paper_to}[/]"
                    )
    ...
    if args.format == "csv":
        ...
        c.print(
            WarpBlocks.panel(
                f"{label} — [#FF8272]{paper_id}[/]", f"[#A5D5FE]{source_title[:60]}[/]"
            )
        )
        if citation_rows:
            c.print(
                WarpBlocks.table(
                    ["Paper ID"], citation_rows, title=f"{len(citation_rows)} Citation(s)"
                )
            )
        else:
            c.print(
                WarpBlocks.panel(
                    "No Citations", "[#8E8E8E]This paper has no citations in the database[/]"
                )
            )
        print(c.file.getvalue(), end="")  # type: ignore[union-attr,attr-defined]
