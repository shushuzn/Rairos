...
def _run_influence(args: argparse.Namespace) -> int:
    ...
    if args.format == "csv":
        ...
        for i, p in enumerate(top_n, 1):
            ...
            print(
                f'{i},"{p.paper_id}","{title_esc}",{p.year},{p.forward_cites},{p.age_years:.1f},{p.velocity:.2f}'
            )
        return 0
    ...
    print()
    print(
        f"Showing {len(top_n)} of {len(all_influence)} papers with >= {args.min_cites} citation(s)"
    )
...
def _show_paper_influence(db, paper_id: str) -> int:
    ...
    print(
        f"    Cited by (forward) : {forward}  → velocity = {forward}/{age} = {velocity:.2f}/y"
        if age > 0
        else f"    Cited by (forward) : {forward}"
    )
    ...
    if age > 0:
        ...
        if velocity >= 10:
            print(
                colored("    🔥 Extremely high velocity (≥10/y) — field-defining", Colors.SUCCESS)
            )
        elif velocity >= 5:
