...
def _build_dedup_parser(subparsers) -> argparse.ArgumentParser:
    ...
    g.add_argument(
        "--batch", action="store_true", help="Auto-merge safe pairs (same DOI), skip the rest"
    )
    p.add_argument(
        "--keep",
...
def _run_dedup(args: argparse.Namespace) -> int:
    ...
    if not pairs:
        c.print(
            WarpBlocks.panel(
                "No Duplicates Found", "[#8E8E8E]No duplicate pairs in the database[/]"
            )
        )
        return 0
    ...
    if args.dry_run and dry_rows:
        ...
        c.print(
            WarpBlocks.panel(
                "Dry Run", f"[#FEFDC2]{len(pairs)} duplicate pair(s) — no changes made[/]"
            )
        )
    c.print()
